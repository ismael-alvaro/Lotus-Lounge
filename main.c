...
// Protótipos de funções (depois a gente vê como tirar daí)
...
{   
    ...
    Texture2D cigarretsTexture = LoadTexture("assets//imagens//cigarro.png");
    Texture2D diskTexture = LoadTexture("assets//imagens//disk.png");
    Texture2D lifeplayer = LoadTexture("assets\\imagens\\vidaplayer.png");
    ...
    {   
        ...
        if (init){
            ...
            Player_main(BASE_WIDTH, BASE_HEIGHT, PLAYER_SPEED, &player, bulletTexture, phaseOneBG, bottleTexture, cigarretsTexture, diskTexture, lifeplayer, &init, level);
        } else {
            SetMusicVolume(musicFase1, 1.0f);
